      ...
      <div @click="toRouter(items)" v-for="items in stores">
        <div class="list">
          ...
          <div class="store-intro">
            ...
            <div class="discount">
              <p><span style="margin-right:0.625rem;" v-if="items.store_point.point_switch == 0?false:true">{{items.store_point.point}}</span><span
                  v-if="items.store_point.point_deduct_switch == 0?false:true">{{items.store_point.max_point_deduct}}</span>
              </p>
              <p><span style="margin-right:0.625rem;" v-if="items.store_love.award==1?true:false">{{items.store_love.award_proportion}}</span><span
                  v-if="items.store_love.deduction==1?true:false">{{items.store_love.deduction_proportion}}</span></p>
            </div>
          </div>
        </div>
      </div>
      <div style="width: 100%;height: 50px;line-height: 50px;" v-if="!show">暂无数据</div>
      <div style="width: 100%;height: 50px;line-height: 50px;" v-if="show" @click="getMoreStores">加载更多</div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    props: ['stores', 'text', 'loading', 'totalPage', 'page'],
    data() {
      return {
        list: [],
    ...
    methods: {
      ...
      getMoreStores() {
        ...
        //   var p = this.fun.bd_encrypt(this.lng, this.lat);
        //
        //   $http
        //     .get(
