...
export default {
  ...
  methods: {
    getDetailInfo() {
      $http.get("plugin.video-demand.api.video-course-goods.get-course-goods-detail", { goods_id: this.$route.params.id }).then(response => {
    ...
    payPost(type) {
      let that = this;
      let json = { "pay_method": type, "amount": this.rewardMoney, "goods_id": this.$route.params.id };
