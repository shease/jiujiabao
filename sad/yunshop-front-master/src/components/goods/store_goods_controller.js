...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      $http.get('goods.goods.get-goods', json, "加载中...").then(function (response) {
        ...
        if (response.result == 1) {
          ...
          wx.miniProgram.postMessage({ data: {goods_title: that.goodsInfo.title} })

          //let store_id=that.fun.isTextEmpty(that.$route.params.store_id)?"":that.$route.params.store_id;
          //todo, 通过接口获取store_id吧
          that.store_id = that.goodsInfo.store_goods.store_id;

          that.footSet(goods_id,that.store_id);//设置足迹数据//描述title路径share
          ...
          console.log('isRent222:', that.isRent);
          console.log('isRentBuyShow11:', that.isRentBuyShow);
          //限时购
    ...
    addCartByO2O() {
      ...
      console.log('that.store_id;', that.store_id);
    ...
    getCart(goods) {
      ...
      that.cartsNum = 0;
      that.cartsTotal = 0;
      $http.get('plugin.store-cashier.frontend.shoppingCart.member-cart.index', {store_id:this.goods_info.store_goods.store_id},"加载中...").then((response) => {
        if (response.result == 1) {
    ...
    addCart1(goods) {
      console.log('addcart...');
      $http.get('plugin.store-cashier.frontend.shoppingCart.member-cart.store', {goods_id: goods.id, total:1, store_id:this.store_id},"加载中...").then((response) => {
        console.log('result', response);
      if (response.result == 1) {
        //goods.buyNum += 1;
        console.log('response.result', response.result);
...
// },
    ...
    init() {
      var that = this;
      $http.get('plugin.store-cashier.frontend.store.get-store-info.get-info-by-store-id', {store_id: this.goods_info.store_goods.store_id}).then((response) => {
        if (response.result == 1) {
        this.getCart();
