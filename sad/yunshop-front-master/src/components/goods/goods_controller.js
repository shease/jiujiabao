...
export default {
  ...
  activated() {
    ...
    this.isFavorite();//获取是否收藏
      ...
      created(){
          $http.get('order.merge-pay.adress', {}, "").then(res=>{
            console.log(res)
            // alert(JSON.stringify(res.data.address))
            this.Adaddress = res.data.address
          })
      },
  mounted() {
    //alert('mount')
      ...
      $http.get('goods.goods.get-goods', json, "加载中...").then(function (response) {
        ...
        if (response.result == 1) {
          ...
          if(!that.fun.isTextEmpty(response.data.has_one_goods_limit_buy)){
            ...
            console.log('isBuy:', that.isBuy);

            that.timeCompare(that.beginTime);

            console.log('isTime:', that.isTime);
