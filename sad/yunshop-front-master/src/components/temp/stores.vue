<template>
	<div>
		...
		<div id="allmap"></div>
		<div v-if="showMore">
			<div style="width: 100%;height: 40px;line-height: 40px;text-align: center;" v-if="loading" @click="getMoreStores">加载更多</div>
	...
	export default {
		...
		methods: {
			...
			toClassify(id) {
				...
				this.$router.push(this.fun.getUrl("o2oCategory", {
					id: id
				}));
			},
			nav(store) {
				...
				geolocation.getCurrentPosition(
					function(r) {
						if (this.getStatus() == BMAP_STATUS_SUCCESS) {
							...
							location.href =
								"https://api.map.baidu.com/direction?origin=" +
								latCurrent +
								"," +
								lngCurrent +
								"&destination=" +
								...
								"," +
								store.longitude +
								"&mode=driving&region=全国&output=html";
				...
				licontent +=
					...
					store.address.detailed_address +
					"</span><br>";
				licontent +=
					"<span><strong>电话：</strong>" + store.mobile + "</span><br>";
				var opts = {
					width: 200,
					height: 80
				};
			...
			toRouter(item) {
				...
				if (item.specify_show == 1) {
					...
					this.$router.push(this.fun.getUrl("HomeSeller", {
						store_id: item.id
					}));
				}
			},
			...
			getMoreStores() {
				...
				if (this.page == this.total_page) {
					...
					// $http.get("plugin.store-cashier.frontend.store.store.get-store-list-to-page", {
					// 	page: that.page,
					// 	kwd: "",
					// 	lng: that.point.lng,
					// 	lat: that.point.lat,
