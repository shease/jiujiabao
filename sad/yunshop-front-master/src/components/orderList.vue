  ...
  <div id="orderList">
    <el-checkbox-group v-model="checkList" @change="multiplePayChange">
      <div class="shop" v-for="order in datasource">
          ...
          <div class="goods">
            ...
            <div class="warp">
              ...
              <div class="price">
                <div class="money">
                  <small>￥</small>
                  {{good.price}}<span v-if="order.plugin_id == 40?true:false">/天</span></div>
                <div> ×{{good.total}}</div>
        ...
        <div class="allbt" v-if="order.status_name== '待取货'">
          <span  @click="operation(order)">退款</span>
          <span  @click="saomiaoMet(order)">待扫描</span>
        </div>
      </div>
    </el-checkbox-group>
    <div class="qrcode">
  ...
  export default {
    ...
    methods: {
      ...
      getCompany() {
        ...
        $http.get(urls, {order_id: that.orderPayId}).then(function (response) {
      ...
      sendGoogs() {
        ...
        if (this.orderType == "supplier") {
          $http.post('plugin.supplier.frontend.order.send', json).then(function (response) {
            if (response.result == 1) {
