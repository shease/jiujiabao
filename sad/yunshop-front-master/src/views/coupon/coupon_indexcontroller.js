...
export default {
    ...
    methods: {
        ...
        goBuy(item) {
            ...
            {
                this.$router.push(this.fun.getUrl('catelist', { "id": JSON.stringify(item.belongs_to_coupon.category_ids),fromHome: 1 }));
            } else if (item.belongs_to_coupon.use_type == 2) {//2指定商品
                this.$router.push(this.fun.getUrl('search', { "coupon": item.belongs_to_coupon.id ,fromHome: 1}));
            }
        },
        delteteItem(item, i) {
            let that = this;
