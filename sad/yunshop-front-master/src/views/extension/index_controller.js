...
export default {
  ...
  created() {
      ...
      $http.get('member.member.getUserInfo',json).then(res=>{
          // alert(JSON.stringify(res))
           $http.get('order.merge-pay.own_money', {uid: res.data.uid})
                  .then((res) => {
                      // alert(22)
                      // alert(JSON.stringify(res))
                      // alert(JSON.stringify(res.data.res))
                      this.fat = res.data.res
           })
       })
 },
  mounted() {
    ...
    setUserInfo() {
      ...
      $http.get('member.member.getUserInfo', json).then(function (response) {
        ...
        if (response.result == 1) {
          ...
          // $http.get('order.merge-pay.own_money', {uid: response.data.uid})
          //       .then((res) => {
          //           alert(JSON.stringify(res))
          // })
        }
        ...
        if (response.result == 1) {
          if (response.data.is_bind_mobile == 1) {
            that.$router.push(that.fun.getUrl('withdrawEditmobile', {}));
             ...
             var ha = that.$refs.qunima.innerText
