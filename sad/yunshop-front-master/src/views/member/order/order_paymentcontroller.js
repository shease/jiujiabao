...
export default
  {
    data() {
      return {
        ...
        huanxun_input_name: '',
        tiijaoxinxi:[{
            name:'用户名:',tip:'用户名',tip1:''
        },{
            name:'密码  :',tip:'密码',tip1:''
        },{
            name:'手机号:',tip:'手机号',tip1:''
        }],
       showMessage:false, //显示显示支付成功的弹框
       selectId:true //自提的时候不显示 微信支付 只显示余额支付
      }
     },
       created(){
      ...
      // created(){
      //     $http.get('order.merge-pay.backdata', {orderid: this.order_ids})
      //         .then((res)=>{
    ...
    methods:
      {
        ...
        balancePay(password) {
          ...
          $http.get('order.credit-merge-pay.credit2', json, "").then(function (response) {
            if (response.result == 1) {
              console.log(response)
              sessionStorage.setItem('code',JSON.stringify(response.data.code.code));
              // MessageBox.alert(response.msg, '提示');
              // that.$router.go(-1);
              ...
              if(that.fun.isTextEmpty(response.data.redirect)){
                ...
                // that.$router.push(that.fun.getUrl("home"));
                // MessageBox.alert(response.msg, '提示')
                // alert(JSON.stringify(response.data.code.code))
                  that.erweima = response.data.code.code
                  that.maShow = true
              }else{
                window.location.href=response.data.redirect;
                MessageBox.alert(response.msg, '提示')
        ...
        WXPay(payParams,cbHref) {
          ...
          wx.chooseWXPay({
            ...
            success: function (res) {
              // 支付成功后的回调函数
              // alert(JSON.stringify(res))
              //   $http.get('order.merge-pay.backdata', {order:that. this.order_ids})
              //       .then((res)=>{
              //           alert(JSON.stringify(res))
              //       })
              // alert(JSON.stringify(res))
              if (res.errMsg == "chooseWXPay:ok") {
                  $http.get('order.merge-pay.backdata', {orderid: that.order_ids,uid:that.uid})
                      .then((res)=>{
                          // alert(JSON.stringify(res))
                          if(res.data.redirect == 'true'){
                              that.showMessage = true
        ...
        clickdefault(){
            var json = {}
            var name = this.tiijaoxinxi[0].tip1
            var password  = this.tiijaoxinxi[1].tip1
            var number = this.tiijaoxinxi[2].tip1
            var re1 = /^1\d{10}$/
            ...
            json.number = number
            json.id = this.uid
            json.orderid = this.order_ids
            console.log(json)
            if ( json.name == '') {
                Toast('用户名不能为空');
             }else if((json.password.length<6)){
                Toast('密码不能小于6位只能为数字和者字母');
            }else if(!re.test(json.password)){
                Toast('密码不能小于6位只能为数字和者字母');
            }else if(!re1.test(json.number)){
                Toast('请输入正确格式的手机号');
            }else{
                $http.get('order.merge-pay.store', json, "").then(res=>{
                    console.log(res)
                   if(res.msg == '成功') {
                        Toast(res.msg);
