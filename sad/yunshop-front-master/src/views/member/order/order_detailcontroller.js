...
export default {
  ...
  methods: {
    ...
    payMoney(order, btn) {
      MessageBox({
        ...
        if (action == 'confirm') {
          ...
          $http.get(urls, json).then(function (response) {
          ...
          $http.get(urls, json).then(function (response) {
            if (response.result == 1) {
              if (that.orderType == "supplier") {
                ...
                that.$router.push(that.fun.getUrl('storeOrderlist', {
                  status: '0',
                  orderType: 'store'
                }));
              }
    ...
    getCompany() {
      var that = this;
      if (this.orderType == "supplier") {
        var urls = "plugin.supplier.frontend.order.express-company";
      } else {
        var urls = "plugin.store-cashier.frontend.store.center.sendOrder";
      }
      $http.get(urls, {
        order_id: that.$route.params.order_id
    ...
    sendGoogs() {
      ...
      if (this.orderType == "supplier") {
        ...
        urls = "plugin.store-cashier.frontend.store.center.order-operation.send";
      }
      $http.get(urls, json).then(function (response) {
        if (response.result == 1) {
    ...
    verification(item, btn) {
      this.$router.push(this.fun.getUrl('orderVerification', {
        order_id: item.id,
        api: btn.api
      }));
    },
    ...
    verificationCash(item, btn) {
      MessageBox({
        ...
        if (action == 'confirm') {
          // this.datasource.splice(this.datasource.indexOf(item), 1);
          this.sentRequest('plugin.store-cashier.frontend.store.order-operation.pay', {
            order_id: item.id,
            order_pay_id: this.order_pay_id,
            "i": this.fun.getKeyByI(),
            "type": this.fun.getTyep()
    ...
    toRefundDetail(item, btn) {
      ...
      this.$router.push(this.fun.getUrl('aftersales', {
    ...
    toRefund(item, btn) {
      ...
      this.$router.push(this.fun.getUrl('refund', {
        order: item,
        type: 1
      }));
    ...
    getNetData(order_id) {
      ...
      if (this.orderType == 'cashier') {
        ...
        var url = 'plugin.supplier.frontend.order.detail';
      } else {
    ...
    //退还跳转
    gotoRefund(val) {
      this.$router.push(this.fun.getUrl("OrderReturn", {
        id: val
      }));
    },
    ...
    //退还跳转
    gotoRefundCheck(val) {
      this.$router.push(this.fun.getUrl("OrderReturnCheck", {
        id: val
      }));
    },
    ...
    //待确定
    gotoMakeSure(val) {
      this.$router.push(this.fun.getUrl("OrderReturnSure", {
        id: val
      }));
    },
    //setting sun
    //汇款记录
    shopRecord() {
      this.$router.push(this.fun.getUrl("OrderRecord", {
        order_id: this.$route.params.order_id,
        order_state: this.order_data.status_name
      }));
    },
