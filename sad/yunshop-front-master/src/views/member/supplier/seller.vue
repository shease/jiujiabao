<template>
  <div id="income">
    <div class="supplier">
      <c-title :hide="false" text='商家联盟'></c-title>
      <div style="height:40px;"></div>
      <div class="banner">

        <img src="../../../assets/images/shangjialianmeng.jpg">
      </div>
      ...
      <div class="main" v-if="supplier_status==1 && !isDiyForm">
        ...
        <div class="welcome">
          ...
          <div class="info">
            ...
            <input type="text" placeholder="请选择地址" class="inp1" v-model="fullAddress" readonly @click="showCity1=true">
        ...
        <template v-for="(item,index) in diydata">
          <template v-if="item.type == 'diyinput'">
            <yd-cell-group>
              <yd-cell-item>
                ...
                <yd-input v-if="item.data.tp_must == 1" slot="right" required v-model="item.value"
                          v-bind:placeholder="item.data.placeholder"></yd-input>
                <yd-input v-if="item.data.tp_must == 0" slot="right" v-model="item.value"
                          v-bind:placeholder="item.data.placeholder"></yd-input>
              </yd-cell-item>
            </yd-cell-group>
          </template>
          ...
          <template v-if="item.type == 'diytextarea'">
            <yd-cell-group :title="item.data.tp_name">
              <yd-cell-item>
                <yd-textarea slot="right" v-model="item.value" :placeholder="item.data.placeholder"
          ...
          <template v-if="item.type == 'diycity'">
            <div>
              <yd-cell-group>
                <yd-cell-item arrow>
                  <span slot="left">{{item.data.tp_name}}：</span>
                  <input slot="right" style="width:74%;" type="text" @click.stop="showCity = true"
                         @click="openCity(item.name)" v-model="item.value" readonly :placeholder="item.data.tp_name">
                </yd-cell-item>
              </yd-cell-group>
          ...
          <template v-if="item.type == 'diypassword'">
            <yd-cell-group>
              <yd-cell-item>
                <span slot="left">{{item.data.tp_name}}：</span>
                <yd-input v-if="item.data.tp_must == 1" slot="right" required v-model="item.value"
                          v-bind:placeholder="item.data.placeholder" type="password"></yd-input>
                <yd-input v-if="item.data.tp_must == 0" slot="right" v-model="item.value"
                          v-bind:placeholder="item.data.placeholder" type="password"></yd-input>
              </yd-cell-item>
            </yd-cell-group>
      ...
      <mt-datetime-picker ref="picker" type="date" year-format="{value} 年" month-format="{value} 月"
                          date-format="{value} 日" @confirm="setDate" v-model="pickerValue" :startDate="startDate">
      </mt-datetime-picker>
...
<script>
  import seller_controller from "./seller_controller";

  export default seller_controller;
</script>
