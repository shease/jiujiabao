...
//  setting sun
  ...
  methods: {
    ...
    getData() {
      ...
      $http.get('plugin.supplier.supplier.controllers.apply.seller-apply', json, "加载中...").then(function (response) {
    ...
    checkEnableDiyForm() {
      let that = this;
      let json = {};
      $http.get('plugin.supplier.supplier.controllers.apply.seller-apply.is-enable-diyform', json, "").then(function (response) {
        console.log(response.data)
        if (response.result == 1) {
    ...
    setApplyData() {
      ...
      $http.get('plugin.supplier.supplier.controllers.apply.seller-apply.apply', json, "提交中...").then(function (response) {
        console.log(response.data)
        if (response.result == 1) {
          that.supplier_status = response.data.status;
        } else {
...
//  setting sun
    ...
    upEvent() {
      this.uploadUrl = this.fun.getRealUrl("plugin.supplier.supplier.controllers.apply.seller-apply.upload", {});
