  ...
  methods:
    {
      ...
      getData2() {
        ...
        $http.get('plugin.supplier.frontend.withdraw.index', json, "获取中...").then(function (response) {
          if (response.result === 1) {
      ...
      withdrawal(type) {
        ...
        $http.get('plugin.supplier.frontend.withdraw.withdraw', {
          apply_type: type,
          member_id: that.$route.params.member_id
        }, '正在提交数据...').then(function (response) {
      ...
      confirmPost(type) {
        ...
        $http.get('finance.balance-withdraw.withdraw', {
