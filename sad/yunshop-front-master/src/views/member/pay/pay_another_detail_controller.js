...
export default {
    ...
    methods: {
        ...
        //微信支付
        getWeChatPayParams() {
            var json = { order_pay_id: this.payId, pid: this.$route.query.pid ? this.$route.query.pid : window.localStorage.getItem("uid") };
            if (window.isMiniprogram) {
              var json = { order_pay_id: this.payId, client_type:2, pid: this.$route.query.pid ? this.$route.query.pid : window.localStorage.getItem("uid") };
            }
