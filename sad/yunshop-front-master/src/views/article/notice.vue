        ...
        <div class="item_bg">
          ...
          <div class="item_lin">
            <p class="item_time" @click="toNoticeInfo(item)">
              {{$vueIsTextEmpty(item.virtual_at)?item.updated_at:item.virtual_at}}</p>
            <p class="item_category" @click="toNoticeByCategory(item.belongs_to_category.id)">
              {{item.belongs_to_category.name}}</p>
          </div>
          <div class="item_img_bg" @click="toNoticeInfo(item)" v-if='item.thumb'><img class="item_img"
                                                                                      :src="item.thumb"/></div>
          <div class="item_img_bg" @click="toNoticeInfo(item)" v-if='!item.thumb'><img class="item_img"
                                                                                       src="../../assets/images/img_default.png"/>
          </div>
          <div class="item_desc" @click="toNoticeInfo(item)">{{item.desc}}</div>
        </div>
      </div>
    </div>
